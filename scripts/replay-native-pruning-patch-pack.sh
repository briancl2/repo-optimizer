...
#
# Usage: bash scripts/replay-native-pruning-patch-pack.sh <repo_path> <output_dir> <target_file> <native_capability>
#
...
REPO="${1:?Usage: replay-native-pruning-patch-pack.sh <repo_path> <output_dir> <target_file> <native_capability>}"
OUTPUT_DIR="${2:?Usage: replay-native-pruning-patch-pack.sh <repo_path> <output_dir> <target_file> <native_capability>}"
TARGET_FILE="${3:?Usage: replay-native-pruning-patch-pack.sh <repo_path> <output_dir> <target_file> <native_capability>}"
NATIVE_CAPABILITY="${4:?Usage: replay-native-pruning-patch-pack.sh <repo_path> <output_dir> <target_file> <native_capability>}"
...
python3 - "$TARGET_FILE" "$NATIVE_CAPABILITY" <<'PY'
from __future__ import annotations

import re
import sys

target_file, native_capability = sys.argv[1:3]
if not re.fullmatch(r"[A-Za-z0-9_./-]+(?:\.md|\.txt|/SKILL\.md)", target_file):
    raise SystemExit("ERROR: target_file must be a .md, .txt, or */SKILL.md path supported by NR-01")
...
|---|---|---:|
| NR-01 | native replacement pruning candidate for native capability \`$NATIVE_CAPABILITY\` in \`$TARGET_FILE\` affected_surface: $TARGET_FILE scan_context={"scanner":"native-pruning-replay","scan_limited":true,"sample":"explicit-target"} evidence_context={"primary_class":"active_doc","source":"owner_issue_80"} | 1 |
EOF
...
python3 - "$RECEIPT" "$STATUS" "$REPO" "$MANIFEST" "$PATCH_DIR" "$GENERATE_LOG" "$VALIDATE_LOG" "$GENERATE_STATUS" "$VALIDATE_STATUS" "$PATCH_COUNT" "$PATCHES_VALID" "$BEFORE_HEAD" "$AFTER_HEAD" "$BEFORE_STATUS" "$AFTER_STATUS" "$UNCHANGED" "$SCRIPT_DIR" "$TARGET_FILE" "$NATIVE_CAPABILITY" <<'PY'
from __future__ import annotations
