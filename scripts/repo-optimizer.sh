...
if command -v copilot >/dev/null 2>&1; then
    ...
    for domain in $OPT_DOMAINS; do
        ...
        prompt_text="Read .agents/${domain}-optimizer.agent.md for instructions. Analyze target repo at $REPO using SCORECARD at $AUDIT_DIR/SCORECARD.json. Write findings as a markdown table to stdout."
        dispatch_ok=false
        if [ "$_has_timeout" = true ]; then
            if (cd "$SCRIPT_DIR/.." && $_to "$OPT_TIMEOUT" copilot --model "$OPT_MODEL" \
                -p "$prompt_text" --allow-all --no-ask-user < /dev/null > "$payload_file" 2>/dev/null); then
                dispatch_ok=true
            fi
        else
            if (cd "$SCRIPT_DIR/.." && copilot --model "$OPT_MODEL" \
                -p "$prompt_text" --allow-all --no-ask-user < /dev/null > "$payload_file" 2>/dev/null); then
                dispatch_ok=true
            fi
        fi
    ...
    if [ "$OPT_OK" -gt 0 ] && [ -f "$AGENTS_DIR/repo-optimizer-critic.agent.md" ]; then
        ...
        critic_prompt="Read .agents/repo-optimizer-critic.agent.md for instructions. Review all domain findings in $OUTPUT_DIR/payloads/. For each finding, assign verdict: APPROVED, DOWNGRADED, or REJECTED. Must reject at least 1. Write verdicts to stdout."
        critic_ok=false
        if [ "$_has_timeout" = true ]; then
            if (cd "$SCRIPT_DIR/.." && $_to "$OPT_TIMEOUT" copilot --model claude-opus-4.6 \
                -p "$critic_prompt" --allow-all --no-ask-user < /dev/null > "$OUTPUT_DIR/critic-verdicts.md" 2>/dev/null); then
                critic_ok=true
            fi
        else
        ...
        if [ "$critic_ok" = true ] && [ -s "$OUTPUT_DIR/critic-verdicts.md" ]; then
            ...
            echo "    Critic review FAILED (findings still available in payloads/)"
        fi
    fi

    # Phase 4: Synthesis
    if [ "$OPT_OK" -gt 0 ] && [ -f "$AGENTS_DIR/repo-optimizer-synthesis.agent.md" ]; then
        echo ""
        echo "  Phase 4: Synthesis..."
        synth_prompt="Read .agents/repo-optimizer-synthesis.agent.md for instructions. Synthesize all domain findings from $OUTPUT_DIR/payloads/ and critic verdicts from $OUTPUT_DIR/critic-verdicts.md into OPTIMIZATION_PLAN.md. Write to $OUTPUT_DIR/OPTIMIZATION_PLAN.md."
        synth_ok=false
        if [ "$_has_timeout" = true ]; then
            if (cd "$SCRIPT_DIR/.." && $_to "$OPT_TIMEOUT" copilot --model claude-opus-4.6 \
                -p "$synth_prompt" --allow-all --no-ask-user < /dev/null > "$OUTPUT_DIR/synthesis-stdout.txt" 2>/dev/null); then
                synth_ok=true
            fi
        else
            if (cd "$SCRIPT_DIR/.." && copilot --model claude-opus-4.6 \
...
" 2>/dev/null || echo "Pre-flight data not available")
    ...
    printf '> Generated by repo-optimizer.sh (pre-flight only, phases 2-4 pending)\n' >> "$OUTPUT_DIR/OPTIMIZATION_PLAN.md"
